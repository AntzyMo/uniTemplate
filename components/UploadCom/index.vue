<template>
	<view class="img-box">
		<view class="preview-box" v-for="(item, index) in imgList" :key="index">
			<image class="preview-img" @click="previewImage([item])" :src="item" mode="aspectFill"></image>
			<text class="cuIcon-close" @tap="DelImg(index)">x</text>
		</view>
		<view class="choose-img " @tap="onChooseImg" v-if="imgList.length < limit">
			<slot v-if="onSlot"></slot>
